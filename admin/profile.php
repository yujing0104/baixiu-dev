...
<body>
  ...
  <script>
    ...
    $('#avatar').on('change',function(){
      ...
      //拿到我们要上传的文件
      var file = files[0]
      //FormData 是HTML5中新增的一个成员,专门配合AJAX操作 用于在客户端和服务端之间传递二进制数据
      var data = new FormData()
      data.append('avatar', file)

      var xhr = new XMLHttpRequest()
      xhr.open('POST', '/baixiu-dev/admin/api/upload.php')
      xhr.send(data)//借助于form data 传递文件

      xhr.onload = function(){
        //console.log(this.responseText)
        $this.siblings('img').attr('src','/baixiu-dev'+this.responseText)
      }
    })
